...
public class YandexSearchHandlerImpl implements MethodCallHandler {
  ...
  private Map<Integer, Session> SearchSessionsById = new HashMap<>();
  public void searchByText(MethodCall call) {
    ...
    searchSession = searchManager.submit(
      ...
      geometryObj,
      searchOptions,
      new YandexSearchListener(listenerId)
    );
    SearchSessionsById.put(listenerId, searchSession);
  }

  public void cancelSearch(MethodCall call) {
      Map<String, Object> params = ((Map<String, Object>) call.arguments);
      final int listenerId = ((Number) params.get("listenerId")).intValue();
...
//    }
  ...
  private class YandexSearchListener implements Session.SearchListener {
    ...
    public YandexSearchListener(int id) {
      listenerId = id;
    }
    private int listenerId;
